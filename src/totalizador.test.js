import { precioNeto, calcularImpuestoEstado, calcularDescuento, calcularCostoEnvio, mostrar } from "./totalizador.js";

test("Calcula el precio neto correctamente", () => {
  expect(precioNeto(10, 5)).toBe(50.00);
});

test("Calcula el impuesto por estado correctamente", () => {
  expect(calcularImpuestoEstado(100, "TX")).toBe(6.25);
  expect(calcularImpuestoEstado(200, "CA")).toBe(16.50);
});

test("Calcula el descuento correctamente", () => {
  expect(calcularDescuento(2000)).toBe(60.00);  // 3% de descuento
  expect(calcularDescuento(5000)).toBe(250.00); // 5% de descuento
});
...
test("Calcula el costo de envío correctamente", () => {
  expect(calcularCostoEnvio(34, 0.2)).toBe(0);      // 34 ítems de 200g c/u
  expect(calcularCostoEnvio(15, 0.8)).toBe(3.5);   // 15 ítems de 800g c/u
  expect(calcularCostoEnvio(13, 2.2)).toBe(5);     // 13 ítems de 2.2Kg c/u
  expect(calcularCostoEnvio(3, 15)).toBe(6);       // 3 ítems de 15Kg c/u
  expect(calcularCostoEnvio(4, 22)).toBe(6.5);     // 4 ítems de 22Kg c/u
  expect(calcularCostoEnvio(2, 64)).toBe(8);       // 2 ítems de 64Kg c/u
  expect(calcularCostoEnvio(3, 87)).toBe(9);       // 3 ítems de 87Kg c/u
});
